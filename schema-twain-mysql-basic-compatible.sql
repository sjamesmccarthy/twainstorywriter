-- Twain Story Builder Database Schema - MySQL Basic Version (Compatible)
-- Created: October 3, 2025
-- This schema mirrors the localStorage structure for easier migration
-- Basic version without complex triggers, stored procedures, views, or CHECK constraints
-- Converted from PostgreSQL to MySQL for maximum compatibility

-- Set SQL mode to strict for better data integrity (MySQL 5.7+ and 8.0+ compatible)
...
CREATE TABLE user_preferences (
    ...
    experimental_features JSON NULL,
    
    -- Recent activity tracking (array of IDs from localStorage)
    recent_books JSON NULL,
    recent_stories JSON NULL,
...
-- Note: Application logic should ensure exactly one parent is set (book_id OR quick_story_id)
CREATE TABLE ideas (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL,
    book_id INT NULL,
...
CREATE TABLE parts (
    ...
    title VARCHAR(500) NOT NULL,
    chapter_ids JSON NULL, -- matches localStorage structure
    story_ids JSON NULL, -- matches localStorage structure
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE note_cards (
    ...
    linked_idea_ids JSON NULL,
    linked_character_ids JSON NULL,
    linked_chapter_ids JSON NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
-- Note: Application logic should allow global activities (both IDs null) or specific parent
